...
const codeSamples: Record<
  Language,
  { raw: string; highlighted: React.ReactNode }
> = {
  python: {
    raw: `import os
...
print("Hello, Watson!")`,
    highlighted: (
      <code>
        ...
        <span className="text-purple-400">from</span> ibm_watson{' '}
        <span className="text-purple-400">import</span> AssistantV2<br />
        <span className="text-purple-400">from</span>{' '}
        ibm_cloud_sdk_core.authenticators{' '}
        <span className="text-purple-400">import</span> IAMAuthenticator<br />
        <br />
        <span className="text-slate-500"># Authenticate</span>
        <br />
        authenticator ={' '}
        <span className="text-yellow-400">IAMAuthenticator</span>(
        os.environ.<span className="text-yellow-400">get</span>(
        <span className="text-green-400">'YOUR_API_KEY'</span>))
        <br />
        assistant = <span className="text-yellow-400">AssistantV2</span>(
        <br />
        {'    '}version=
        <span className="text-green-400">'2021-06-14'</span>,<br />
        {'    '}authenticator=authenticator<br />)
        <br />
        assistant.
        <span className="text-yellow-400">set_service_url</span>(
        <span className="text-green-400">'YOUR_SERVICE_URL'</span>)<br />
        <br />
        <span className="text-slate-500"># Greet Watson</span>
        <br />
        <span className="text-yellow-400">print</span>(
        <span className="text-green-400">"Hello, Watson!"</span>)<br />
      </code>
    ),
  },
...
console.log("Hello, Watson!");`,
    highlighted: (
      <code>
        <span className="text-purple-400">const</span> AssistantV2 ={' '}
        <span className="text-yellow-400">require</span>(
        <span className="text-green-400">
          'ibm-watson/assistant/v2'
        </span>);<br />
        <span className="text-purple-400">const</span> {'{'} IamAuthenticator{' '}
        {'}'} = <span className="text-yellow-400">require</span>(
        <span className="text-green-400">'ibm-watson/auth'</span>);<br />
        <br />
        <span className="text-slate-500">// Authenticate</span>
        <br />
        <span className="text-purple-400">const</span> assistant ={' '}
        <span className="text-purple-400">new</span>{' '}
        <span className="text-yellow-400">AssistantV2</span>({'{'}
        <br />
        {'  '}version:{' '}
        <span className="text-green-400">'2021-06-14'</span>,<br />
        {'  '}authenticator:{' '}
